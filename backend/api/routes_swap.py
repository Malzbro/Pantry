...
from db.models import Plan, PlanMeal, HouseholdMember
...
def swap_meal(
    ...
    if request.plan_id is not None and request.meal_index is not None:
        plan = (
            db.query(Plan)
            .join(HouseholdMember, HouseholdMember.household_id == Plan.household_id)
            .filter(Plan.id == request.plan_id, HouseholdMember.user_id == user_id)
            .first()
        )
        if plan is not None:
            payload = dict(plan.response_payload)
            meals_list = list(payload.get("meals", []))
            if 0 <= request.meal_index < len(meals_list):
                meals_list[request.meal_index] = result.model_dump(mode="json")
                payload["meals"] = meals_list
                plan.response_payload = payload

                meal_row = (
                    db.query(PlanMeal)
                    .filter(PlanMeal.plan_id == plan.id, PlanMeal.meal_index == request.meal_index)
                    .first()
                )
                if meal_row:
                    meal_row.recipe_id = recipe.id
                else:
                    db.add(PlanMeal(plan_id=plan.id, meal_index=request.meal_index, recipe_id=recipe.id))
